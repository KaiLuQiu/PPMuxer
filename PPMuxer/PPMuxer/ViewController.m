...
-(void)addButClick
{
    ...
    [WphotoVC setSelectPhotosBack:^(NSMutableArray *videoUrlArr) {
        [WphotoVCs dismissViewControllerAnimated:YES completion:^{
            PPMuxerController *MuxerController = [self.storyboard instantiateViewControllerWithIdentifier:@"MuxerController"];
            [MuxerController passViewController:videoUrlArr];
            [self presentViewController:MuxerController animated:YES completion:nil];
        }];
    }];
